...
class TFRecordCreator:
    ...
    def _serialize_sample(self, sample):
        ...
        support_dict = {k: np.float32(v) for k, v in support_dict.items()}
        ...
        if self.is_continuous_time:
            for step in range(sample.get_number_of_steps()):
                ...
                for support_name, support in support_dict.items():
                    tf_record_dict[support_name] = _bytes_feature(support[step].tobytes())
            ...
            for support_name, support in support_dict.items():
                tf_record_dict[support_name] = _bytes_feature(support.tobytes())
